 ...
 *
 * <pre>
 * $model=MyTree::model()->findBy_id(1);
 * echo $model->pathText;
 * </pre>
 *
...
class XTreeBehavior extends CActiveRecordBehavior
{
    ...
    public function getRootId()
    {
        ...
        $criteria = new EMongoCriteria(array(
            'condition' => array($this->parent_id=>'0'),
        ));


        $root = Content::model()->findOne($criteria);
    ...
    public function getMenuItems($id=null,$showRoot=true)
    {
        ...
        if ($showRoot===false)
        {
            ...
            $criteria = new EMongoCriteria(array(
                'condition' => array($this->parent_id=>$rootId),
                'sort'=>array($this->sort=>'asc'),
            ));
    ...
    public function getTreeItems($id=null,$showRoot=true)
    {
        ...
        else
        {
            ...
            $criteria = new EMongoCriteria(array(
                'condition' => array('id'=>$rootId),
                'sort'=>array($this->sort=>'asc'),
            ));

            $model=$owner->findOne($criteria);
            if($model===null)
                throw new CException('The requested tree does not exist.');
            else
                $items[]=$model->getTreeSubItems();
    ...
    public function fillTree($id=null, $showRoot=true)
    {
        ...
        {
            $model=$owner->with($this->getWidth())->findBy_id($rootId);
            if($model===null)
                throw new CException('The requested tree does not exist.');
            $items[]=$this->formatTreeItem($model);
        }
    ...
    public function beforeSave($event)
    {
        ...
        $newParent=$owner->findBy_id($parentId);
        if($newParent===null)
        {
